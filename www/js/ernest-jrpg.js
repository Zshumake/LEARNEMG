...
class ErnestJRPG {
    constructor() {
        ...
        // --- CLIENT-SIDE REFLEXES (Zero-API Personality) ---
        this.reflexes = {
            'mri': {
    ...
    resetIdleTimer() {
        ...
        // Only roast if chat is OPEN
        if (this.ui.wrapper.classList.contains('active')) {
            this.idleTimer = setTimeout(() => this.triggerIdleRoast(), 60000); // 60 seconds
        }
    }

    triggerIdleRoast() {
        if (this.isThinking) return; // Don't interrupt thinking
        ...
        if (this.currentPersonaId === 'earl') {
            ...
            this.addToChat('earl', `*${randomRoast}*`);
        }
    }

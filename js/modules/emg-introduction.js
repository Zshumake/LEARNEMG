...
function playTerminologyPodcast() {
    ...
    if (window.playModulePodcast) {
        window.playModulePodcast('emg-introduction', 'emg-terminology');

        // Open player if needed (usually handled by playModulePodcast, but ensuring UI state)
        if (typeof window.togglePodcastPlayer === 'function') {
            const player = document.getElementById('podcast-player-overlay');
            if (!player || player.style.display === 'none') {
                // window.togglePodcastPlayer(); // playModulePodcast handles this
            }
        }
    } else {

...
### Round 14 (finish the web a11y sweep — closes the audit's #1 finding)
A `/code-review` of the whole session's refactor work came back clean (no CRITICAL/HIGH; 0 console.log/TODO/eval/secrets; modal listener leak-free; all files <800 lines). Then finished the largest open a11y item:
- [x] **Keyboard-operable clickable divs** (`c803711`): added `role="button" tabindex="0"` to **36 clickable `<div>`s** (clinical difficulty/case cards, EMG Indicated/Not-Indicated choices, 12 nerve-pathway items, plexus tabs, CEL category/section/test cards, study-card/needle/report toggles, podcast banner). The central Enter/Space ActionBus keydown handler (added round 9) already routes activation; this added the markup so they're focusable + announced as buttons (visible text = accessible name). With 0 inline onclick remaining, every clickable element is now covered (~39 total keyboarded).
- [x] **Input labels**: `aria-label` on **8 placeholder-only inputs** (glossary/diagnosis/muscle searches, `final-diagnosis` textarea, and the dynamic report-writing fields via `${label}`).
- Verified in-browser: clinical dashboard renders 49 case cards with `role=button` + `data-action=startCase`, `final-diagnosis` labelled "Final diagnosis", keyboard activation routes through the proven central handler, **no console errors**, no visual regression. Deployed + live.
- Remaining from the "what else" list (working down): boot-graph lazy-defer (perf), add tests, new EMG cases (content).

### Round 15 (new EMG cases → already done; doc fix + welcome-screen redesign)
- [x] **New EMG cases — surfaced as already-done, not duplicated.** The CLAUDE.md "candidate cases not yet added" (C6 radic, GBS, Saturday-night palsy, brachial plexopathy, LEMS) were ALL already in the DB (added 2026-06-11); the library is ~50 cases. Wrote nothing; **fixed the stale CLAUDE.md note** (`4e32c4d`) to reflect reality + the genuinely-niche remaining gaps.
- [x] **`/code-review`** of the whole session's refactor work — came back clean (no CRITICAL/HIGH; 0 console.log/TODO/eval/secrets; modal listener leak-free; files <800 lines).
- [x] **Welcome-screen redesign** (`a6244de`, deployed). First swing was a bold dark "EDX workstation" concept — user rejected it; reverted cleanly (was never committed). Second take per user steer ("light, friendly, polished, same theme"):
  - Pure-CSS polish layer (`css/welcome-polish.css`, no markup changes): fresher layered-green atmosphere + faint texture, refined frosted card (layered shadow, hairline edge, green top accent), theme-tinted green-gradient title, single clean speech-bubble tail (killed the legacy duplicate), refined CTA (arrow slide + idle pulse), gentle staggered entrance, reduced-motion safe.
  - Dropped the mascot podium shadow after iterating: the Ernest/Earl animations swing translateY -40..+60px across 9 states, so a fixed ground shadow can't stay under him without per-state syncing — not worth the fragility.
  - **Funnier persona dialogue**: each persona now has its own randomized voice (Ernest happy-go-lucky, Earl grumpy/rude) — 5 greetings + 5 poke replies each, EMG-flavored. Earl previously had none + borrowed Ernest's. Also stopped `ErnestCore.switchPersona` from overwriting the rotating greeting with the static description.

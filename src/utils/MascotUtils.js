/**
 * MascotUtils Utility
 * Centralizes the generation of full-body SVG versions of Ernest and Earl
 * with unique ID scoping ONLY for definitions (gradients, clips, filters) 
 * to prevent transparency issues while maintaining global CSS animations.
 */
export const MascotUtils = {
    /**
     * Generates a full-body mascot SVG with unique scoping for definitions.
     * @param {string} personaId - 'ernest' or 'earl'
     * @param {string} instanceId - Unique suffix for IDs
     * @param {string} extraClasses - Optional extra CSS classes
     * @returns {string} SVG HTML string
     */
    getFullSVG(personaId, instanceId = '', extraClasses = '') {
        const id = instanceId || Math.random().toString(36).substr(2, 6);
        
        if (personaId === 'earl') {
            return `
<svg id="earl-${id}" class="earl-svg-char ${extraClasses}" viewBox="0 0 500 550" preserveAspectRatio="xMidYMax meet" xmlns="http://www.w3.org/2000/svg">
    <defs>
    ...
    <g id="earl-body-group">
        <path id="body-chassis-${id}" d="M 110 160 C 110 110, 140 100, 180 100 L 320 100 C 360 100, 390 110, 390 160 C 390 200, 350 210, 330 240 C 320 255, 320 280, 320 300 L 320 480 C 320 530, 280 540, 250 540 C 220 540, 180 530, 180 480 L 180 300 C 180 280, 180 255, 170 240 C 150 210, 110 200, 110 160 Z" 
        ...
        // Default Ernest SVG
        return `
<svg id="ernest-${id}" class="ernest-svg-char ${extraClasses}" viewBox="0 0 500 500" preserveAspectRatio="xMidYMax meet" xmlns="http://www.w3.org/2000/svg">
    <defs>
        <style>
            /* Local specific styles that might need scoping can stay here, 
               but main classes like .ernest-outline stay plain for global CSS. */
    ...
    <g transform="translate(45, 10)">
        ...
        <g>
            ...
            <path d="M 100 130 C 80 130 70 145 70 160 L 70 200 C 70 230 140 250 140 280 L 140 450 C 140 500 280 500 280 450 L 280 280 C 280 250 350 230 350 200 L 350 160 C 350 145 340 130 320 130 Z" 
                fill="#55595f" filter="url(#cel-shading-${id})"/>
            ...
            <path d="M 100 130 L 320 130 C 340 130 350 145 350 160 L 350 185 C 350 210 270 230 210 230 C 150 230 70 210 70 185 L 70 160 C 70 145 80 130 100 130 Z" 
                fill="#80858b" filter="url(#cel-shading-${id})"/>
            <path d="M 140 435 Q 210 445 280 435 L 280 450 C 280 500 140 500 140 450 Z" 
                fill="#606469" filter="url(#cel-shading-${id})"/>
            <path d="M 100 130 C 80 130 70 145 70 160 L 70 200 C 70 230 140 250 140 280 L 140 450 C 140 500 280 500 280 450 L 280 280 C 280 250 350 230 350 200 L 350 160 C 350 145 340 130 320 130 Z" 
        ...
        <g id="cartoon-face">
            ...
            <g class="ernest-eyes">
                ...
                <g class="ernest-eyes-closed" style="display: none;">
            ...
            <g class="ernest-mouth-container">
                <g class="ernest-mouth-open" transform="translate(78.5, 108.75) scale(0.65)">
                    ...
                    <path d="M 185 340 Q 210 315 235 340 C 235 365 185 365 185 340 Z" fill="#ff7675" stroke="#2a2d34" stroke-width="2" clip-path="url(#mouth-cut-${id})"/>
                ...
                <g class="ernest-mouth-closed" transform="translate(78.5, 108.75) scale(0.65)" style="display: none;">
...
if (typeof window !== 'undefined') {
    window.MascotUtils = MascotUtils;
}
